...
public class CourseMapper implements BeanAndModelMapper<CourseBean, Course> {
    ...
    public Course fromBeanToModel(CourseBean bean) {
        return new Course(bean.getCourseName(), bean.getSlots(), bean.getRemainingSlots(), bean.getDuration(), bean.getLevel(), bean.getDay(), bean.getHour());
    }

    @Override
    public CourseBean fromModelToBean(Course model) {
        return new CourseBean( model.getCourseName(), model.getSlots(), model.getRemainingSlots(), model.getDuration(), model.getLevel(), model.getDay(), model.getHour());
    }
}
