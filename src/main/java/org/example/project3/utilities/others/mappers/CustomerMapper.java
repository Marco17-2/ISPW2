...
import com.example.bodybuild.beans.CustomerBean;
import com.example.bodybuild.model.Customer;
...
public class CustomerMapper implements BeanAndModelMapper<CustomerBean,Customer> {
    ...
    public Customer fromBeanToModel(CustomerBean bean) {
        if(bean.getName() != null)
            return new Customer(credentialsMapper.fromBeanToModel(bean.getCredentialsBean()), bean.getName(), bean.getSurname(), bean.isOnline());
        else
    ...
    public CustomerBean fromModelToBean(Customer model) {
        if(model.getName() != null)
            return new CustomerBean(credentialsMapper.fromModelToBean(model.getCredentials()), model.getName(), model.getSurname(), model.isOnline());
        else
